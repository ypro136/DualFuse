...
size_t fat32_cluster_to_LBA(FAT32 *fat, uint32_t cluster) {
...
#define IS_UPPERCASE(c)                                                        \
  (((c) >= 'A' && (c) <= 'Z') || ((c) >= '0' && (c) <= '9'))
int fat32_is_short_filename_possible(char *filename, size_t len) {
  if (len > (8 + 1 + 3))
    return -1;
...
// TODO: add for() when compiler optimizations are in control
//******************* */
...
void fat32_LFNmem_cpy(uint8_t *lfnName, FAT32LFN *lfn, int index) {
...
int fat32_SFN_to_normal(uint8_t *target, FAT32DirectoryEntry *dirent) {
  int i;
...
unsigned long fat32_unix_time(unsigned short fat_date, unsigned short fat_time) {
