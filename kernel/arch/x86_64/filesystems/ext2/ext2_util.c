...
size_t ext2BlockSizeCalculate(Ext2 *ext2, size_t raw) {
  uint32_t blocks = CEILING_DIVISION(raw, ext2->blockSize);
  ...
  if (blocks < base) {
    ...
    size_t singly_needed = CEILING_DIVISION(remaining, perSpecial);
...
uint32_t ext2BlockFetch(Ext2 *ext2, Ext2Inode *ino, uint32_t inodeNum,
    ...
    spinlock_cnt_read_acquire(&ext2->WLOCKS_BLOCK_BITMAP[group]);
    ...
    spinlock_cnt_read_release(&ext2->WLOCKS_BLOCK_BITMAP[group]);
    return result;
}
...
void ext2BlockAssign(Ext2 *ext2, Ext2Inode *ino, uint32_t inodeNum,
    ...
    uint32_t group = INODE_TO_BLOCK_GROUP(ext2, inodeNum);
    spinlock_cnt_write_acquire(&ext2->WLOCKS_BLOCK_BITMAP[group]);
    ...
    if (curr < 12) {
        ...
        if (!ino->blocks[12]) {
            spinlock_cnt_write_release(&ext2->WLOCKS_BLOCK_BITMAP[group]);
        ...
        set_disk_bytes((void *)control->tmp1, tmp1block,
                       ext2->blockSize / SECTOR_SIZE);
    } 
    /*else if (curr < baseDoubly) {
        if (!ino->blocks[13])
...
uint32_t ext2BlockFindL(Ext2 *ext2, int group, uint32_t amnt) {
    ...
    for (int i = 0; i < ext2->blockSize && !found; i++) {
