...
static inline struct boundary_tag* split_tag( struct boundary_tag* tag )
{
	...
	struct boundary_tag *new_tag = (struct boundary_tag*)((unsigned int)tag + sizeof(struct boundary_tag) + tag->size + bootloader.hhdmOffset);	
...
void *malloc(size_t size)
{
	...
	ptr = (void*)((unsigned int)tag + bootloader.hhdmOffset + sizeof( struct boundary_tag ) );
...
void free(void *ptr)
{
	...
	liballoc_lock();
		...
		tag = (struct boundary_tag*)((unsigned int)ptr + bootloader.hhdmOffset - sizeof( struct boundary_tag ));
