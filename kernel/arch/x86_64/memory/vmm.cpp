...
void virtual_memory_manager_initialize(
    uint64_t memory_map_Total,
    uint64_t memory_map_entry_count,
    struct limine_memmap_entry** memory_map_entries,
    uint64_t _hhdmOffset)
{
    hhdmOffset = _hhdmOffset;
    size_t targetPosition =
        CEILING_DIVISION(hhdmOffset - memory_map_Total - VMM_POS_ENSURE, PAGE_SIZE)
        * PAGE_SIZE;

    _virtual.ready = false;
    _virtual.mem_start = targetPosition;
    _virtual.BitmapSizeInBlocks =
        CEILING_DIVISION(memory_map_Total, BLOCK_SIZE);
    _virtual.BitmapSizeInBytes =
        CEILING_DIVISION(_virtual.BitmapSizeInBlocks, 8);

    uint64_t pagesRequired =
        CEILING_DIVISION(_virtual.BitmapSizeInBytes, BLOCK_SIZE);
    _virtual.Bitmap = (uint8_t *)virtual_allocate(pagesRequired);
    memset(_virtual.Bitmap, 0, _virtual.BitmapSizeInBytes);

    // The bitmap is placed inside the HHDM region and is never used
    // outside of boot / locked physical allocations, so no extra
...
void *virtual_allocate(int pages) {
    size_t phys = physical_allocate(pages);     // now internally locked
...
bool virtual_free(void *ptr, int pages) {
    size_t phys = (size_t)virtual_to_physical((uint64_t)ptr);
    if (!phys) {
        printf("[vmm::free] Could not find physical address! virt{%x}\n", ptr);
        Halt();
    }
    physical_free(phys, pages);                 // now internally locked
    return true;
}
