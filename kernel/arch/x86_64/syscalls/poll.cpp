...
void pollInstanceRing(size_t key, int epollEvent) {
  spinlock_acquire(&LOCK_POLL_ROOT); // <- very important thing
...
size_t epollCreate1(int flags) {
  ...
  spinlock_acquire(&LOCK_LL_EPOLL);
  Epoll *epoll = LinkedListAllocate(&dsEpoll, sizeof(Epoll));
  spinlock_release(&LOCK_LL_EPOLL);
  epoll->timesOpened = 1;
  spinlock_acquire(&LOCK_POLL_ROOT);
...
void epollCloseNotify(OpenFile *fd) {
  ...
  while (browseEpoll) {
    spinlock_acquire(&browseEpoll->LOCK_EPOLL);
    ...
    if (browseEpollWatch) {
      ...
      LinkedListRemove(&browseEpoll->firstEpollWatch, sizeof(EpollWatch),
                       browseEpollWatch);
      spinlock_release(&browseEpoll->LOCK_EPOLL);
      spinlock_release(&LOCK_LL_EPOLL);
      return;
    }
...
size_t epollCtl(OpenFile *epollFd, int op, int fd, struct epoll_event *event) {
  ...
  default:
    printf("[epoll] Unhandled opcode{%d}\n", op);
    Halt(); // should've already been checked!
    break;
  }
...
size_t epollWait(OpenFile *epollFd, struct epoll_event *events, int maxevents,
  ...
  do {
    spinlock_acquire(&epoll->LOCK_EPOLL);
    spinlock_acquire(&LOCK_POLL_ROOT); // these two for pollInstanceWait()
    EpollWatch *browse = (EpollWatch *)epoll->firstEpollWatch.firstObject;
...
size_t ppoll(struct pollfd *fds, int nfds, struct timespec *timeout,
  ...
  size_t epollRet = poll(
      fds, nfds,
      timeout ? (CEILING_DIVISION(timeout->tv_nsec, 1000000) + timeout->tv_sec * 1000)
              : -1);
  if (sigmask)
    syscallRtSigprocmask(SIG_SETMASK, &origmask, 0, sigsetsize);
...
void selectRemainingTime(struct timeval *timeout, size_t elapsed_ms) {
  ...
  size_t timeout_ms =
      timeout->tv_sec * 1000 + CEILING_DIVISION(timeout->tv_usec, 1000);
...
size_t select(int nfds, uint8_t *read, uint8_t *write, uint8_t *except,
  ...
  int toZero = CEILING_DIVISION(nfds, 8);
  if (read)
  ...
  size_t res = poll(
      comp, compIndex,
      timeout ? (timeout->tv_sec * 1000 + CEILING_DIVISION(timeout->tv_usec, 1000))
              : -1);
