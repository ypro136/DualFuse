...
Futex *futexFind(size_t phys) {
  spinlock_acquire(&LOCK_AVL_FUTEX);
...
cleanup:
  spinlock_acquire(&futex->LOCK_PROP);
  spinlock_release(&LOCK_AVL_FUTEX);
  return futex;
}
...
size_t futexSyscall(uint32_t *addr, int op, uint32_t value,
  ...
  if (op & FUTEX_CLOCK_REALTIME) {
    printf("[futex] Seriously, it uses FUTEX_CLOCK_REALTIME: op{%x}\n", op);
    Halt();
  }
  ...
  if (op == FUTEX_WAIT || op == FUTEX_WAKE || op == FUTEX_REQUEUE) {
    if (!addr || ((size_t)addr % 4) != 0)
      return ERR(EINVAL);
    phys = virtual_to_physical((size_t)addr);
    assert(phys);
  ...
  if (op == FUTEX_REQUEUE) {
    if (!addr2 || ((size_t)addr2 % 4) != 0)
      return ERR(EINVAL);
    phys2 = virtual_to_physical((size_t)addr2);
    assert(phys2);
  ...
  case FUTEX_WAIT: {
    ...
    spinlock_acquire(&newfutex->LOCK_PROP);
    if (asleep->awoken)
      ret = 0;
    ...
    spinlock_release(&newfutex->LOCK_PROP);
  ...
  case FUTEX_REQUEUE: {
    ...
    spinlock_release(&futex2->LOCK_PROP);
    spinlock_release(&futex->LOCK_PROP);
  ...
  default:
    printf("[futex] Invalid operation{%x}\n", op);
    Halt();
    return ERR(ENOSYS);
    break;
  }

  printf("[futex] Dead-end. It requires some serious effort to end up here!\n");
  assert(false);
}