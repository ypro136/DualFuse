...
static size_t syscallPipe(int *fds) { return pipe_open(fds); }
...
static size_t syscallClone(uint64_t flags, uint64_t newsp, int *parent_tid,
  ...
  Task *newTask =
      task_fork(currentTask->syscallRegs,
...
static size_t syscallFork() {
  return task_fork(currentTask->syscallRegs, currentTask->syscallRsp, 0, true)
      ->id;
}

#define SYSCALL_VFORK 58
static size_t syscallVfork() {
  Task *newTask = task_fork(currentTask->syscallRegs, currentTask->syscallRsp,
                           CLONE_VM, false);
  int   id = newTask->id;
...
static size_t syscallWait4(int pid, int *wstatus, int options,
  ...
  if (pid != -1) {
    ...
    KilledInfo *browse = (KilledInfo *)currentTask->dsChildTerminated.firstObject;
    while (browse) {
      if (browse->pid == pid)
        break;
      browse = (KilledInfo *)browse->_ll.next;
    }
    target = browse;
